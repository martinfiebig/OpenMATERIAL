...
* Mapping one _material property_ file per _material_ name in the _3D model_.
  In the overview image above, the material "asphalt" may for example be mapped to a _material property_ file "aphalt.xomp".
* Mapping a color code from a dedicated {THIS_STANDARD} assignment texture to a _material property_ file, enabling detailed differentiation between _material properties_.
  ...
  They use the same uv-map as the visual texture.
