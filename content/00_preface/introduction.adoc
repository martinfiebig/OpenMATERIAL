...
3D objects defined in the 07_geometry/geometry-index.adoc[Geometry] chapter act as a connecting link between {THIS_STANDARD} and other ASAM standards:
...
* ASAM OpenSCENARIO XML is a file format created to describe dynamic content in driving and traffic simulators, with a primary focus on complex maneuvers involving multiple entities like vehicles and pedestrians.
  {THIS_STANDARD} geometry chapter defines node structures and metadata for these object classes to seamlessly integrate ASAM OpenSCENARIO XML definitions with a _3D model_.
